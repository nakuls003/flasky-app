...
def get_comments():
    ...
    if pagination.has_prev:
        prev = url_for('api.get_comments', page=pagination.prev_num)
    if pagination.has_next:
        next = url_for('api.get_comments', page=pagination.next_num)
...
def get_post_comments(id):
    ...
    prev, next = None, None
    if pagination.has_prev:
        prev = url_for('api.get_post_comments', page=pagination.prev_num)
    if pagination.has_next:
        next = url_for('api.get_post_comments', page=pagination.next_num)
    return jsonify({'comments': [comment.to_json() for comment in comments],
                    'prev': prev,
                    'next': next,
                    'count': pagination.total
